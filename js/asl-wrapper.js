window._ASL_load = function() {
    ...
    window.ASL.instances = {
        ...
        get: function(b, a) {
            ...
            if ("undefined" === typeof a) {
                ...
                for (var c = 0; c < this.instances.length; c++)
                    this.instances[c].o.id == b && a.push(this.instances[c]);
                return 0 < a.length ? a : !1
            }
            for (c = 0; c < this.instances.length; c++)
                if (this.instances[c].o.id == b && this.instances[c].o.iid == a)
                    return this.instances[c];
            return !1
        },
        set: function(b) {
            if (this.exist(b.o.id, b.o.iid))
                return !1;
        ...
        exist: function(b, a) {
            ...
            for (let c = 0; c < this.instances.length; c++)
                if (this.instances[c].o.id == b && ("undefined" === typeof a || this.instances[c].o.iid == a))
                    return !0;
            return !1
        },
        clean: function() {
            ...
            b.forEach(function(c) {
                "undefined" !== typeof a.instances[c] && (a.instances[c].destroy(),
    ...
    window.ASL.initializeSearchByID = function(b) {
        ...
        a.forEach(function(e, h) {
            d(".asl_w_container_" + h).forEach(function(f) {
                var g = d(f).parent();
                g.is("a") && (f = document.createElement("div"),
                g = g.get(0),
            ...
            d(".asl_m_" + h).forEach(function(f) {
                let g = d(f);
                if ("undefined" != typeof g.get(0).hasAsl)
                    return ++c,
                    !0;
                f.hasAsl = !0;
                ++c;
    ...
    window.ASL.getInstances = function() {
        ...
        d(".asl_init_data").forEach(function(a) {
            if ("undefined" === typeof a.dataset.asldata)
                return !0;
            let c = WPD.Base64.decode(a.dataset.asldata);
            if ("undefined" === typeof c || "" == c)
                return !0;
            b[a.dataset.aslId] = JSON.parse(c)
        });
        return b
    }
    ;
    window.ASL.initialize = function(b) {
        if ("undefined" == typeof ASL.version)
            return !1;
        if (window.IntersectionObserver)
            if (ASL.script_async_load || ASL.init_only_in_viewport) {
                if (b = document.querySelectorAll(".asl_w_container"),
                b.length) {
                    let a = new IntersectionObserver(function(c) {
                        c.forEach(function(e) {
                            e.isIntersecting && (ASL.initializeSearchByID(e.target.dataset.id),
    ...
    window.ASL.initializeHighlight = function() {
        if (this.highlight.enabled)
            return this.highlight.data.forEach(function(b) {
                ...
                null !== c && "" != c.trim() && (a = 0 < d(a).length ? a : "body",
                d(a).highlight(c.trim().split(" "), {
                    element: "span",
                    className: "asl_single_highlighted",
                    wordsOnly: b.whole,
                    excludeParents: ".asl_w, .asl-try"
                }),
                a = d(".asl_single_highlighted"),
                b.scroll && 0 < a.length && (a = a.offset().top - 120,
                c = d("#wpadminbar"),
    ...
    window.ASL.ready = function() {
        if ("complete" === document.readyState || "loaded" === document.readyState || "interactive" === document.readyState)
            ...
            d(document).on("DOMContentLoaded", this.initialize)
    }
    ;
    window.ASL.loadScriptStack = function(b) {
        let a;
        0 < b.length && (a = document.createElement("script"),
        a.src = b.shift().src,
        a.onload = function() {
    ...
    window.ASL.init = function() {
        ASL.script_async_load ? window.ASL.loadScriptStack(ASL.additional_scripts) : "undefined" !== typeof WPD.ajaxsearchlite && window.ASL.ready()
    }
    ;
